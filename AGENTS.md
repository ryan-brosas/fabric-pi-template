...
- **Pi runtime lifecycle** (`.pi/prompts/`, `.pi/artifacts/`) — what the template ships. Each Pi lifecycle prompt takes an explicit `<slug>` and reads/writes `.pi/artifacts/<slug>/{PLAN,TODO,PROGRESS,DECISIONS}.md` (lowercase-hyphen, validated). The "no shared `.active` pointer, no 'latest' inference" rule applies to this layer — concurrent sessions on different slugs stay disjoint. Only `/verify` writes terminal verified status; `/ship` records implementation but cannot claim completion.
