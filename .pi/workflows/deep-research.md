...
> **Pi execution binding:** The TypeScript blocks run inside `fabric_exec` as real governed dispatch (not pseudocode); the primary brokers evidence and synthesizes the final report. Full doctrine — GLM 5.2 12-pool (`makora/zai-org/GLM-5.2-NVFP4` + `umans/umans-glm-5.2`), read-only lanes (`openai-codex/gpt-5.4-mini`, alt `claude-bridge/claude-haiku-4-5`), judgment (`openai-codex/gpt-5.6-sol`), role routes, required_skills, board states (assigned → running → returned → verified | failed), worker-distrust / primary-sole-integrator — is in `.pi/docs/fabric-tuning.md` (kernel in `APPEND_SYSTEM.md`).
# deep-research
