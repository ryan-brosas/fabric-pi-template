...
- Full git-safety rules (no force-push main/master, no destructive restoration, `main`
  only with `master` synced) live in `AGENTS.md` — this file is preferences, not policy.
...
- Authoritative home: `.pi/` (config, prompts, artifacts, context packet). Reference
  scaffold: `.opencode/`.
