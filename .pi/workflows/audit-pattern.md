...
> **Pi execution binding:** The phase prompts run inside `fabric_exec` as real governed dispatch — read-only fan-out (`explore`/`scout`) on small-model lanes (`openai-codex/gpt-5.4-mini`, alt `claude-bridge/claude-haiku-4-5`) and judgment (`review`) on `openai-codex/gpt-5.6-sol`; the implementation `general` route (GLM 12 pool `makora/zai-org/GLM-5.2-NVFP4` + `umans/umans-glm-5.2`) is not dispatched here. The full dispatch doctrine — GLM pool, role routes, `required_skills`, board states (`assigned → running → returned → verified | failed`), worker-distrust / primary-sole-integrator — lives in `.pi/docs/fabric-tuning.md` (kernel in `APPEND_SYSTEM.md`).
# audit-pattern
