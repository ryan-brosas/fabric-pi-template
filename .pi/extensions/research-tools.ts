...
function text(content: string, details: Record<string, unknown> = {}) {
  ...
  return {
    ...
    details: { ...details, truncation: truncated },
  };
}
...
export default function researchTools(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    label: "Context7",
    description:
      "Resolve library IDs and query current library documentation. Use resolve before query; do not use for local code or general web research.",
      ...
      if (operation === "resolve") {
        ...
        return text(
          results
            ...
            .map(
              (lib, i) =>
                `${i + 1}. ${lib.name ?? lib.title} -> ${lib.id}\n${lib.description ?? ""}`,
            )
            .join("\n\n") || `No libraries found for ${args.libraryName}`,
          { operation, count: results.length },
        );
      }
      if (operation === "query") {
        if (!args.libraryId?.trim() || !args.topic?.trim())
          throw new Error("libraryId and topic are required for query");
        const url = new URL(`${CONTEXT7_API}/context`);
        url.searchParams.set("libraryId", args.libraryId);
        url.searchParams.set("query", args.topic);
  ...
  pi.registerTool({
    ...
    label: "grep.app",
    description:
      "Search literal production code patterns in public GitHub repositories through grep.app. Use for real-world library usage, not local code or prose research.",
    parameters: Type.Object({
      query: Type.String(),
    ...
    async execute(_id, args, signal) {
      ...
      const response = await fetch(url, {
        headers: { Accept: "application/json", "User-Agent": "Pi/1.0" },
        signal,
      });
      ...
      const formatted = hits
        .slice(0, limit)
        .map((hit, i) => {
          const snippet = String(hit.content?.snippet ?? "")
            .replace(/<[^>]*>/g, "")
            .replace(/&lt;/g, "<")
            .replace(/&gt;/g, ">")
            .replace(/&amp;/g, "&")
            ...
            .slice(0, 8)
            .join("\n");
          return `${i + 1}. ${hit.repo?.raw ?? hit.repo}\nFile: ${hit.path?.raw ?? hit.path}\n${snippet}`;
        })
        .join("\n\n");
      return text(formatted || `No results found for ${args.query}`, {
        count: hits.length,
        elapsedMs: data.took ?? data.time,
      });
    },
  });
}
